    ...
    <div className="flex h-full flex-col bg-white">
      ...
      <nav className="flex-1 px-3 py-4 space-y-1">
        {NAV.filter((i) => i.show).map((item) => {
          ...
          return (
            <NavLink key={item.to} to={item.to} onClick={onClickItem} data-testid={item.testid}
              className={({ isActive }) =>
                `flex items-center gap-3 rounded-lg px-4 py-2.5 text-sm font-medium transition-colors ${
                  isActive ? "bg-brand-light text-[#036A87]" : "text-gray-600 hover:bg-gray-50 hover:text-gray-900"
                }`}>
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC]">
        ...
        <div className="lg:hidden fixed inset-0 z-40">
          ...
          <div className="absolute inset-y-0 right-0 w-72 bg-white shadow-xl">
            <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200">
              ...
              <button onClick={() => setOpen(false)} className="p-2 text-gray-500"><X className="h-5 w-5" /></button>
            </div>
      ...
      <header className="lg:hidden sticky top-0 z-30 bg-white border-b border-gray-200 flex items-center justify-between px-4 h-14">
        ...
        <button onClick={() => setOpen(true)} data-testid="mobile-menu-btn" className="p-2 rounded-lg text-gray-700 hover:bg-gray-100">
