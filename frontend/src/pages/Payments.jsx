...
const PaymentBadge = ({ semester }) => {
  ...
  return (
    <span
      className={`inline-flex text-xs px-2 py-0.5 rounded-full border ${styles[semester] || "bg-gray-50 text-gray-600 border-gray-200"}`}
    >
...
export default function Payments() {
  ...
  const exportPayments = async () => {
    try {
      const params = { search: debouncedQ, academicYear: year, semester: sem };
      ...
      const columns = [
        ...
        { label: "المبلغ", value: (p) => p.amount },
        { label: "إجمالي المستحق", value: (p) => p.totalPayable },
        { label: "المتبقي", value: (p) => p.totalRemaining },
        { label: "تاريخ الدفع", value: (p) => p.paymentDate },
        { label: "سجل بواسطة", value: (p) => p.createdByName },
      ];
      downloadCsv("payments.csv", columns, response.data);
    } catch {
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm">
        <div className="p-4 border-b border-gray-200 flex flex-wrap gap-3">
          ...
          <input
            ...
            placeholder="سنة دراسية (مثال 2026-2027)"
          ...
          <select
            ...
            className="rounded-lg border border-gray-300 px-3 py-2 text-sm"
            data-testid="payments-semester-filter"
          >
              ...
              {loading ? (
                ...
                items.map((p) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 font-semibold text-gray-900 tabular-nums">
                      {(p.totalPayable ?? 0).toLocaleString("ar-EG")}
                    </td>
                    <td className="px-4 py-3 font-semibold text-amber-700 tabular-nums">
                      {(p.totalRemaining ?? 0).toLocaleString("ar-EG")}
                    </td>
                    <td className="px-4 py-3 text-gray-600">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setEditing({ ...editing, paymentDate: e.target.value })
                    }
                    className="w-full rounded-lg border border-gray-300 px-3 py-2 focus:ring-2 focus:ring-[#04CDF9]"
                  />
                </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40">
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full p-6 border border-gray-200">
            ...
            <div className="flex items-center justify-end gap-2">
              ...
              <button
                onClick={doDelete}
                data-testid="confirm-payment-delete-btn"
