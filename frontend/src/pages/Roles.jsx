...
export default function Roles() {
  ...
  const load = async () => {
    ...
    try {
      ...
      toast.error("تعذر تحميل الأدوار");
    }
    setLoading(false);
  };
  useEffect(() => {
  ...
  const doDelete = async () => {
    try {
      await api.delete(`/roles/${confirmId}`);
      ...
      <div className="flex items-end justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            إدارة أدوار المستخدمين ومنح الصلاحيات
          </p>
        </div>
        {has("roles.create") && (
          <button
              ...
              roles.map((r) => (
                ...
                >
                  <td className="px-4 py-3 font-medium text-gray-900 flex items-center gap-2">
                    <ShieldCheck className="h-4 w-4 text-[#04CDF9]" /> {r.name}
                  </td>
                    ...
                    <div className="inline-flex gap-1">
                      ...
                      {has("roles.delete") && (
                        <button
                          ...
                          data-testid={`delete-role-${r.name}`}
          ...
          <div className="bg-white rounded-xl shadow-xl max-w-3xl w-full my-8 border border-gray-200">
            <div className="p-6 border-b border-gray-200">
              <h3 className="text-lg font-bold text-gray-900">
                {editing.id ? "تعديل دور" : "إضافة دور"}
              </h3>
            </div>
              ...
              <div>
                ...
                <div className="space-y-4">
                  {Object.entries(groups).map(([res, list]) => {
                    const all = list.every((p) =>
                      editing.permissions.includes(p.name),
                    );
                    return (
                      <div
                        key={res}
                      ...
                      >
                        <div className="flex items-center justify-between mb-2">
                          ...
                          <button
                            ...
                            className="text-xs text-[#036A87] hover:underline"
                          >
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-1">
                          {list.map((p) => {
                            const checked = editing.permissions.includes(
                              p.name,
                            );
                            return (
                              <label
                                key={p.name}
                                className="inline-flex items-center gap-2 text-sm text-gray-700 py-1 cursor-pointer"
                              >
                                <input
                                  type="checkbox"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40">
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full p-6 border border-gray-200">
            ...
            <div className="flex items-center justify-end gap-2">
              ...
              <button
                onClick={doDelete}
                data-testid="confirm-role-delete-btn"
