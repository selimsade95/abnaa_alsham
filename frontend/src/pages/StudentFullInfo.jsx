  ...
  return (
    <div className="space-y-6" data-testid="full-info-page">
      <div className="flex items-end justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            المعلومات الكاملة • كود الطالب:{" "}
            <span className="font-mono">{data.code || "—"}</span>
          </p>
        </div>
        {has("students.update") && (
          <button
            onClick={() => nav(`/students/${id}/full-information/edit`)}
            data-testid="fullinfo-edit-btn"
          ...
          >
            <Pencil className="h-4 w-4" /> تعديل المعلومات الكاملة
          </button>
        )}
      </div>
      ...
      <Section title="بيانات الطالب الأساسية">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6">
          ...
          <Row
            ...
            value={
              data.currentClass
                ? `${data.currentClass.grade || ""} — ${data.currentClass.section || ""}`
                : fi.classSection
            }
          />
      ...
      <Section title="الأسرة والإخوة">
        ...
        <div className="mt-3">
          <Row label="مع من يسكن الطالب" value={fi.livesWith} />
        </div>
      </Section>
      ...
      <Section title="بيانات والدي الطالب">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <Row
              label="حالة الأب"
              value={f.alive ? "على قيد الحياة" : "متوفى"}
            />
          ...
          <div>
            ...
            <Row
              label="حالة الأم"
              value={m.alive ? "على قيد الحياة" : "متوفى"}
            />
      ...
      <Section title="بيانات القيد المدرسي">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6">
          ...
          <Row
            label="الصف الحالي"
            value={data.currentClass?.name || s.newClass}
          />
      ...
      <Section title="المدارس والمراكز السابقة">
        {(data.previousEducation || []).length > 0 ? (
          <table className="min-w-full text-sm">
            <thead className="bg-gray-50 text-xs text-gray-500 uppercase">
              <tr>
                <th className="px-3 py-2 text-right">الصف2</th>
                <th className="px-3 py-2 text-right">اسم المدرسة</th>
                <th className="px-3 py-2 text-right">اسم المركز</th>
            ...
            <tbody>
              {data.previousEducation.map((e, i) => (
                <tr key={i} className="border-t border-gray-100">
                  ...
                  <td className="px-3 py-2">{e.centerName}</td>
      ...
      <Section title="حالة الطوارئ">
        ...
        />
        <Row
          label="اسم مالك الرقم"
          value={data.general?.emergencyContact?.name}
        />
        <Row
          label="علاقة مالك الرقم بالطالب"
