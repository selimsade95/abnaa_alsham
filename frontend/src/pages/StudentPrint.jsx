...
const Section = ({ title, children }) => (
  <div className="print-card border border-gray-300 rounded-md p-3 mb-3">
    <div className="text-[11px] uppercase font-bold text-gray-700 border-b border-gray-300 pb-1 mb-2">
      {title}
    </div>
...
export default function StudentPrint() {
  ...
  if (!data)
    return <div className="p-6 text-gray-500">جاري التحضير للطباعة...</div>;
      ...
      <div className="max-w-[210mm] mx-auto p-8 print-area">
        ...
        <div className="no-print flex items-center justify-end gap-2 mb-3">
          <button
            ...
            className="rounded-lg bg-[#04CDF9] text-white px-4 py-2 text-sm"
          >
            طباعة
          </button>
          <button
        ...
        <Section title="معلومات الطالب">
          <div className="grid grid-cols-2 gap-x-4">
            ...
            <Row
              ...
              value={
                data.currentClass?.name
                  ? `${data.currentClass.name}${data.currentClass.section ? " — " + data.currentClass.section : ""}`
                  : s.newClass
              }
            />
            ...
            />
            {s.orphan && (
              <Row label="نوع وثيقة اليتم" value={s.orphanDocType} />
            )}
            ...
            <Row
              label="عادات دائمة"
              value={
                s.permanentHabits ? s.permanentHabitsDetails || "نعم" : "لا"
              }
            />
          </div>
        </Section>
        ...
        <div className="print-card border border-gray-300 rounded-md p-4 mt-3">
          ...
          <p className="text-[12px] leading-relaxed mb-4">
            ...
            حال ثبوت عدم صحتها، أتحمل المسؤولية وأوافق على القرار الذي تتخذه
            إدارة المدرسة.
          </p>
