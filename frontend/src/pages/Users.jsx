                  ...
                  <td className="px-4 py-3">
                    <div className="flex flex-wrap gap-1">
                      {(u.roles || []).map((rid) => (
                        <span
                          key={rid}
                          className="inline-flex text-xs px-2 py-0.5 rounded-full bg-brand-light text-[#036A87] border border-[#04CDF9]/30"
                        >
                          {roleName(rid)}
                        </span>
                      ))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40">
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full p-6 border border-gray-200">
            <h3 className="text-lg font-bold text-gray-900 mb-4">
              {editing.id ? "تعديل مستخدم" : "إضافة مستخدم"}
            </h3>
            <form onSubmit={save} className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full rounded-lg border border-gray-300 px-3 py-2 focus:ring-2 focus:ring-[#04CDF9] disabled:bg-gray-50"
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  {editing.id ? "كلمة مرور جديدة (اختياري)" : "كلمة المرور"}
                </label>
                <input
                  data-testid="user-password-input"
                  type="password"
                  required={!editing.id}
                  minLength={editing.id ? 0 : 4}
